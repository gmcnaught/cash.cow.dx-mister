...
static int emit(blt_emitter_t *e, const blt_cmd_t *c)
{
    ...
    /* [Cash Cow DX] Pack on the stack, then write the ring as 8 aligned 32-bit
     * stores. On device the ring is the /dev/mem window, strongly-ordered unless
     * mem_wc is loaded, where every store is its own bus transaction:
     * blt_pack_cmd straight into the ring issued 32 byte stores per command.
     * Same 32 bytes (the host is little-endian, like the wire). */
    uint32_t w[BLT_CMD_BYTES / 4];
    blt_pack_cmd(c, (uint8_t *)w);
    uint8_t *dst = e->ring + pos;
    if (((uintptr_t)dst & 3u) == 0) {
        volatile uint32_t *d = (volatile uint32_t *)dst;
        d[0] = w[0]; d[1] = w[1]; d[2] = w[2]; d[3] = w[3];
        d[4] = w[4]; d[5] = w[5]; d[6] = w[6]; d[7] = w[7];
    } else {
        memcpy(dst, w, BLT_CMD_BYTES);
    }
    e->cmd_count++;
    return 0;
}
