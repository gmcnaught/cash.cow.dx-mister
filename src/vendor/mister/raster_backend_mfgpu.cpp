...
// [Cash Cow DX] Lookup hint for g_texcache. Every sprite quad looks its cropped
// region up, and the linear scan over 256 x 48-byte slots (until the hit) was
// the bulk of mf_draw's cost on the A9. A direct-mapped table remembers the slot
// each (key, rect) was last found or inserted in. The hint is only ever a
// candidate: it is accepted when that slot still holds a used entry with the
// same (key, rect), else the original scan runs. Inserts happen only after a
// scan missed, so at most one used slot matches a (key, rect) and the hinted
// slot IS the one the scan would return. Stale hints (evict, invalidate,
// reinit) therefore need no maintenance.
enum { MF_TEX_HINT_N = 4096 };   // power of two; ~16x the slot count keeps collisions rare
static uint16_t g_tex_hint[MF_TEX_HINT_N];   // slot + 1; 0 = no hint
static inline uint32_t mf_tex_hint_idx(uint32_t key, int rx, int ry, int rw, int rh) {
    uint32_t h = key * 0x9E3779B1u;
    h ^= ((uint32_t)rx | ((uint32_t)ry << 16)) * 0x85EBCA77u;
    h ^= ((uint32_t)rw | ((uint32_t)rh << 16)) * 0xC2B2AE3Du;
    return (h ^ (h >> 15)) & (MF_TEX_HINT_N - 1);
}
// The scan's match predicate, verbatim (uint16 fields compared against int).
static inline bool mf_tex_entry_is(const MfTexEntry &e, uint32_t key, int rx, int ry, int rw, int rh) {
    return e.used && e.key == key && e.rx == rx && e.ry == ry && e.rw == rw && e.rh == rh;
}
// Slot of the used entry for (key, rect), or -1: the same answer as the scan.
static int mf_tex_find(uint32_t key, int rx, int ry, int rw, int rh) {
    const uint32_t h = mf_tex_hint_idx(key, rx, ry, rw, rh);
    const int s = (int)g_tex_hint[h] - 1;
    if (s >= 0 && mf_tex_entry_is(g_texcache[s], key, rx, ry, rw, rh)) return s;
    for (int i = 0; i < MF_TEX_CACHE_N; i++)
        if (mf_tex_entry_is(g_texcache[i], key, rx, ry, rw, rh)) {
            g_tex_hint[h] = (uint16_t)(i + 1);
            return i;
        }
    return -1;
}
...
static blt_surface_ref_t mf_upload_and_cache(uint32_t key, int w, int h,
    ...
    g_tex_hint[mf_tex_hint_idx(key, rx, ry, w, h)] = (uint16_t)(slot + 1);   // next lookup skips the scan
    return ref;
}
...
static blt_surface_ref_t stage_texture(uint32_t key, const RTexture *t, bool *out_has_key) {
    ...
    {
        const int i = mf_tex_find(key, 0, 0, tw, th);
        if (i >= 0) {
...
//
// [Cash Cow DX] (int)floorf / (int)ceilf are libm calls on ARMv7 (no vrint), 4
// per sprite quad. Every caller passes u0,v0,u1,v1 from mf_clamp01 bboxes seeded
// with 1.0/0.0, so the products are in [0, page] (or -0.0), where
// (int)floorf(y) == (int)y and (int)ceilf(y) == (int)y + ((float)(int)y < y).
// Checked against libm for every float in [-0, 2^31) (work/fabric_eq/crop_eq.c).
static inline int mf_floor_nonneg_i(float y) { return (int)y; }
static inline int mf_ceil_nonneg_i(float y)  { int i = (int)y; return i + ((float)i < y ? 1 : 0); }
static void mf_crop_rect(const RTexture *t, float u0, float v0, float u1, float v1,
                         int *rx, int *ry, int *rw, int *rh) {
    int x0 = mf_clampi(mf_floor_nonneg_i(u0 * t->w) - 1, 0, t->w - 1);
    int y0 = mf_clampi(mf_floor_nonneg_i(v0 * t->h) - 1, 0, t->h - 1);
    int x1 = mf_clampi(mf_ceil_nonneg_i (u1 * t->w) + 1, 1, t->w);
    int y1 = mf_clampi(mf_ceil_nonneg_i (v1 * t->h) + 1, 1, t->h);
    *rx = x0; *ry = y0;
    *rw = (x1 - x0 < 1) ? 1 : x1 - x0;
    *rh = (y1 - y0 < 1) ? 1 : y1 - y0;
}
...
static blt_surface_ref_t stage_texture_region(uint32_t key, const RTexture *t,
    ...
    *out_rx = rx; *out_ry = ry;
    {
        const int i = mf_tex_find(key, rx, ry, rw, rh);
        if (i >= 0) {
            g_texcache[i].lru = ++g_lru_clock;
            *out_has_key = g_texcache[i].has_key;
...
static void mf_emit_group(const blt_surface_ref_t &tex, int tw, int th,
    ...
    // [Cash Cow DX] Colour word memoized across vertices and calls: identical
    // output to bvtx_to_blt, 4 roundings saved per vertex (raster_backend_convert.h).
    static RbcRgbaMemo rgba_memo;
    for (int i = 0; i < nverts; i++) {
        g_vtxscratch[i] = bvtx_to_blt_with_rgba(&verts[i], tw, th, bvtx_rgba_memo(&rgba_memo, &verts[i]));
