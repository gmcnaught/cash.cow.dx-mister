...
// [Cash Cow DX] Inline lroundf. On ARMv7 lroundf is a libm call (glibc's generic
// C version; VFPv3 has no round-half-away conversion, vcvta is ARMv8), and this
// file makes 8 of them per vertex -- ~1% of the game's main thread. vcvt.s32.f32
// truncates toward zero, and x - trunc(x) is exact in float (Sterbenz for
// |x| >= 1, trivially below), so testing that fraction against +/-0.5 is
// round-half-away-from-zero exactly. Outside int32 (and +/-inf) glibc returns
// (long)x, the same saturating vcvt, which the INT32_MAX/MIN guards preserve; NaN
// converts to 0 in both. Checked bit-exact against glibc's armhf lroundf for all
// 2^32 inputs (cash.cow.dx-mister work/fabric_eq/lround_eq.c).
static inline int32_t rbc_lroundf(float x) {
    int32_t i = (int32_t)x;
...
static inline int16_t rbc_pack_fx(float px) {
    ...
    return (int16_t)rbc_lroundf(fx);
}

// The per-vertex colour word alone. Split out so a caller converting many
// vertices can reuse it while r,g,b,a repeat (see bvtx_rgba_memo).
static inline uint32_t bvtx_rgba(const BVtx *v) {
    return BLT_RGBA(rbc_lroundf(v->r * 255.0f), rbc_lroundf(v->g * 255.0f),
                    rbc_lroundf(v->b * 255.0f), rbc_lroundf(v->a * 255.0f));
}

// bvtx_to_blt with the colour word supplied (it depends only on r,g,b,a).
static inline blt_vtx_t bvtx_to_blt_with_rgba(const BVtx *v, int tex_w, int tex_h, uint32_t rgba) {
    blt_vtx_t o;
    o.x = rbc_pack_fx(v->x);
    o.y = rbc_pack_fx(v->y);
    o.u = (uint16_t)rbc_lroundf(rbc_clampf(v->u, 0.0f, 1.0f) * (float)tex_w * 16.0f);
    o.v = (uint16_t)rbc_lroundf(rbc_clampf(v->v, 0.0f, 1.0f) * (float)tex_h * 16.0f);
    o.rgba = rgba;
    o._rsvd = 0;
    return o;
}
...
static inline blt_vtx_t bvtx_to_blt(const BVtx *v, int tex_w, int tex_h) {
    return bvtx_to_blt_with_rgba(v, tex_w, tex_h, bvtx_rgba(v));
}

// [Cash Cow DX] One-entry memo of bvtx_rgba, keyed on the exact bit patterns of
// r,g,b,a. Nearly every vertex of a frame carries the same modulate (usually
// white): Godot's canvas modulate is per rect, identical on all 4 corners. The
// colour word is a pure function of those 16 bytes, so a bitwise-equal key
// reuses the identical word (bitwise, so -0.0 vs 0.0 and NaN payloads never
// alias).
static_assert(offsetof(BVtx, a) == offsetof(BVtx, r) + 3 * sizeof(float), "r,g,b,a must be contiguous");
struct RbcRgbaMemo {
    float    key[4];
    uint32_t rgba;
    bool     valid;
};
static inline uint32_t bvtx_rgba_memo(RbcRgbaMemo *m, const BVtx *v) {
    if (!m->valid || memcmp(m->key, &v->r, sizeof m->key) != 0) {
        memcpy(m->key, &v->r, sizeof m->key);
        m->rgba = bvtx_rgba(v);
        m->valid = true;
    }
    return m->rgba;
