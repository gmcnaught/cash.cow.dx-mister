...
`mister-port.toml`, `dist/` and `scripts/`.
...
| `external/mister-hybrid-platform/` | Git submodule under its own licences, including the `mem_wc` driver (GPL-2.0, from skmp/minicast) and `MiSTer_hybrid` (MiSTer-devel/Main_MiSTer, GPL-3.0, plus the platform's hook; the upstream commit is pinned in its `device/main-hook/build-hps.sh`). |
